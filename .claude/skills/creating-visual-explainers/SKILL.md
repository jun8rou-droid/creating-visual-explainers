...
description: Generates an illustrated HTML page about any topic and deploys it to surge.sh. Triggered by requests like "図解を作って", "図解を生成して", "このトピックを図解して", or "図解してデプロイして". For report-tool automation, use Plan mode first—pass the four planning pillars (trigger, source, processing, delivery) plus target report HTML, then have a sub-agent review the plan before building.
---
...
- **図解とは** — テキストだけでは掴みにくい**関係・流れ・構造**を、**ビジュアルで一度見て理解できる**ようにした表現である。確認・把握が**一目でしやすい**ことを目的に図（図形・矢印・ラベル・並び・色分けなど）を用いる。
- **求めるもの** — **わかりやすい図**（何が主役か、前後関係が追えるか）。**絵や図の芸術的品質・細部の作り込みは必須としない**。装飾より**情報が伝わること**を優先する。
- **模範回答の位置づけ** — `model-answer.html` は「高品質なイラスト」のお手本ではなく、**読みやすい情報の載せ方・レイアウト・パターン**の参考とする。同じ密度や装飾を無理に再現する必要はない。**同等以上に一目で腹落ちする**ことを優先する。

## 報告ツール自動化の計画（プランモード優先）

いきなり実装や画面作りに入らず、**プランモード**でまず計画を立てる。完成形の共有には**レポートHTML（モック／一枚絵）**を使う。

### 計画で決める4パーツ

| パーツ | 決めること |
|--------|------------|
| **トリガー** | **いつ・何をきっかけに**処理が動くか（スケジュール、イベント、手動、しきい値など） |
| **ソース元** | **データがある場所**（基幹、DWH、スプレッドシート、API、ファイル置き場など） |
| **処理する場所** | **加工してレポートにする場所**。例: 前段で数値集計を確定し、**Anthropic API（Claude）** で要約・通知文を生成。呼び出し元はクラウド関数・社内ジョブなど。 |
| **届ける先** | **完成レポートを届ける先**（メール、チャット、ポータル、共有ストレージ、印刷フローなど） |

### 計画の立て方

1. **プランモードを選ぶ**（実装モードに入らない）。
2. 上記 **4パーツ**（ユーザーが整理した内容でもよい）と **レポートHTML**（できあがりの見た目・項目の「絵」）を AI に渡し、**自動化の計画書**（流れ、依存、失敗時、セキュリティ観点の抜けチェック）を書かせる。
3. **サブエージェント**に計画をレビューさせる（抜け、矛盾、運用負荷、セキュリティ）。
4. レビューを反映して**計画を修正する**。
5. 計画が固まってから、実装・連携・HTMLの手直しに進む。

図解HTMLの生成・公開ワークフローは下記「ワークフロー」と併用できる（レポートの「完成形の絵」が先にあり、それに沿ってパイプラインを設計するイメージ）。
...
模範回答がデザインガイドラインの代わりになる。パーツ一覧やルールではなく、実物から読み取る。**図の美しさより、一目で構造がわかるか**を真似る（上記「図解の定義」）。
...
- **図の優先順位** — **視覚的に一発で伝わるか**（ラベル・矢印・順序・グルーピング）を最優先。**見た目のリッチさ・イラスト調の完成度**は二の次。シンプルな枠線・アイコン・矢印で足りるならそれでよい。
- **概論 → 各論** — いきなり詳細に入らない。全体像を見せてから個別の話に入る
- **一文はシンプルに** — 1文に要素を詰め込みすぎない。主語と述語が追いやすい**短めの文**を基本にする。長くなるなら**句読点で分割**し、読み返さなくても意味が取れるリズムを優先する。
- **内容は少し掘り下げる** — 言い換えや同じ観点の繰り返しだけで終わらせない。**なぜそうか**、**その結果どうなるか**、**具体例が一つ**、**注意点や限界**など、腹落ちに必要な**一歩深い**説明を足す。情報の**点数**は減らさない（下の「簡潔にまとめすぎない」とセット）。
...
- **簡潔にまとめすぎない** — 理解に必要な**情報量**は削らない。腹落ちするまで丁寧に説明する（**文は短く**、**中身は浅くしない**）。
...
- **出典の優先度（公式を軸に、段階的に広げる）** — 事実・仕様・数値の根拠は**第1優先**を原則とし、文脈・用語・事例は**第2・第3**で補足してよい。第3だけを仕様の根拠にしない。主張ごとに「一番強い1本」を基本とし、無理に出典を増やさない。
  - **第1優先（本文の柱・仕様・数値）** — 公式ドキュメント、RFC・標準規格、製品マニュアル、政府・学会の一次データ、公式ブログ・リリースノート、当該プロジェクト公式の README / 仕様。定義・バージョン・API 挙動・数値はここで裏付けできるならここで完結させる。
  - **第2優先（補足・文脈・現場の言い方）** — 編集プロセスのある専門メディア、調査会社の公開レポート、認知できる専門家のブログ（一次を引用しているもの）、教科書・論文・技術書（版・章が特定できるもの）。出典表記で「解説」「背景」など種別が分かるようにしてよい。
  - **第3優先（慎重・種別明示）** — MDN などコミュニティメンテのドキュメント、Qiita / Zenn / Stack Overflow、Wikipedia。概要・慣用・手順の紹介にとどめ、**仕様の最終根拠にはしない**。争点・数値は第1で再確認する。コミュニティ由来は「出典: ○○（コミュニティ投稿）」のように分かるラベルにする。
  - **使わない（原則）** — 著者・根拠が不明なまとめサイト、コピペ感の強いコンテンツファーム、公式・一次と矛盾する二次情報をそのまま「事実」として書くこと。
